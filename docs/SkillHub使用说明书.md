...
SkillHub 的主要能力包括全局 Skill 管理、文件与仓库导入、集合组织、项目独立配置、同步预览、冲突保护和同步撤销。3.2.0 同时提供可选的 SkillOps Agent 辅助模块，用于在这些既有流程上帮助检索、检查和维护 Skill；手动管理和项目同步仍可独立使用。
...
全局库中的 Skill 可以编辑、分类、导入、删除和同步。隐藏目录 `.skill-hub/` 用于保存索引、集合、导入快照和回收站，不作为普通 Skill 显示。
...
- **全局库模式**：未选择项目时使用。用于浏览、搜索、导入、创建、编辑、分类和删除全局 Skill。
...
- 同名未托管文件不会被静默覆盖。
- AI 优化、集合冲突、高风险项和受限 Bundle 文件分别要求确认。
